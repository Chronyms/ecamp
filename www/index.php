<?php
   ...
   */
    ...
    if ($debug) {
        echo $_user_camp->auth_level . '<br /><br />';
    }
    ...
    if (!in_array($_page->app, $valid_app)) {
        error_message("Keine gültige Applikation: " . $_page->app);
    }
  
    // Erlaubte Kommandos (Files) lesen    z.B. index.php?app=home&cmd=index
    if (is_file($app_dir."/".$_page->app."/config.php")) {
        include($app_dir."/".$_page->app."/config.php");
    }
  
    // Kommando überprüfen & checken, ob Zugriff erlaubt
    if (!isset($security_level[$_page->cmd])) {
        error_message("Keine gültiges Kommando: ".$_page->cmd);
    }
    if ($security_level[$_page->cmd] > $_user_camp->auth_level) {
        error_message("Keine Berechtigung für ausgewählten Befehl!");
    }
    ...
    $index_content['main'] = "";
    if (file_exists($app_dir."/".$_page->app."/index.php")) {
        include($app_dir."/".$_page->app."/index.php");
    }
    if (is_file($app_dir."/".$_page->app."/".$_page->cmd.".php")) {
        include($app_dir."/".$_page->app."/".$_page->cmd.".php");
    } else {
        error_message("Datei zum Kommando konnte nicht gefunden werden. Kommando: ".$_page->cmd);
    }
