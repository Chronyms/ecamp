 ...
 */
    ...
    class page
    {
        ...
        public function addCssConfig($config)
        {
        ...
        public function addJsConfig($config)
        {
            foreach ($config as $file => $type) {
                if ($type == "app") {
        ...
        public function loadFileDependency($file)
        {
            if (! is_file($file)) {
                return;
            }
            ...
            while (!feof($fh)) {
                ...
                foreach ($depTags as $dep) {
                    ...
                    if ($type == "") {
                        $type = end(pathinfo($path, PATHINFO_EXTENSION));
                    }
    ...
    class user_camp
    {
        ...
        public function load_data($data)
        {
            ...
            if (isset($data['auth_level'])) {
                $this->auth_level = $data['auth_level'];
            }
        }
    }
    ...
    class camp
    {
        ...
        public function event_checklist($id)
        {
        ...
        public function event_comment($id)
        {
        ...
        public function event_document($id)
        {
        ...
        public function event_instance($id)
        {
        ...
        public function event_responsible($id)
        {
        ...
        public function todo_user_camp($id)
        {
    ...
    class news
    {
        ...
        public function add2camp($title, $text, $date = 0, $camp_id = 0)
        {
            global $_camp;
        ...
        public function add2user($title, $text, $date = 0, $user_id = 0)
        {
            if (!$date) {
                $date = time();
            }
            ...
            while (array_key_exists($date, $news)) {
                $date ++;
            }
        ...
        public function remove($key, $user_id = 0)
        {
            global $_user;
            if (!$user_id) {
                $user_id = $_user->id;
        ...
        public function save($news, $user_id = 0)
        {
            global $_user;
            if (!$user_id) {
                $user_id = $_user->id;
