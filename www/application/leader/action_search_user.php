 ...
 */
    ...
    $select_arg = array("0");
    if (!empty($scoutname)) {
        $select_arg[] = " (scoutname LIKE '$scoutname%')*1 + (scoutname LIKE '$scoutname')*15 ";
    }
    if (!empty($firstname)) {
        $select_arg[] = " (firstname LIKE '$firstname%')*1 + (firstname LIKE '$firstname')*15 ";
    }
    if (!empty($surname)) {
        $select_arg[] = " (surname LIKE '$surname%')*2 + (surname LIKE '$surname')*30 ";
    }
    if (!empty($mail)) {
        $select_arg[] = " (mail LIKE '$mail%')*5 + (mail LIKE '$mail')*100 ";
    }
