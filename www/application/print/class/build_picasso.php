 ...
 */
    ...
    class print_build_picasso_class
    {
        ...
        public function set_orientation($orientation = 'L')
        {
        ...
        public function get_number_of_page($subcamp)
        {
            if ($this->orientation == 'L') {
        ...
        public function get_number_of_rows($subcamp, $page)
        {
            $rows = floor(count($subcamp->day) / $this->get_number_of_page($subcamp));
            if ($page <= (count($subcamp->day) % $this->get_number_of_page($subcamp))) {
                $rows++;
            }
            return $rows;
        }
        ...
        public function category_list($pdf)
        {
        ...
        public function background($pdf)
        {
            ...
            for ($h = 0; $h < 24; $h+=2) {
                if ($h < 2) {
                    $pdf->Rect(10, $yy, $this->pw - 20, $baseh, 'F');
                    $yy += 2 * $baseh;
                }
                if ($h >= 2 && $h < 18) {
                    $pdf->Rect(10, $yy, $this->pw - 20, 2 * $baseh, 'F');
                    $yy += 4 * $baseh;
                }
                if ($h >= 18) {
                    $pdf->Rect(10, $yy, $this->pw - 20, $baseh, 'F');
            ...
            for ($h = 0; $h < 40; $h++) {
                if ($h < 2) {
                    $time = gmdate('H:i', ($h+5)*3600) . " - " . gmdate('H:i', ($h+5+1)*3600);
                }
                if ($h >= 2 && $h < 34) {
                    $time = gmdate('H:i', ($h/2+1+5)*3600) . " - " . gmdate('H:i', ($h/2+1.5+5)*3600);
                }
                if ($h >= 34) {
                    $time = gmdate('H:i', ($h+8+5)*3600) . " - " . gmdate('H:i', ($h+9+5)*3600);
                }
                
                $pdf->SetXY(10, 30 + $h * $baseh);
                $pdf->drawTextBox($time, 20, $baseh, 'C', 'M', 0);
                $pdf->SetXY($this->pw - 30, 30 + $h * $baseh);
                $pdf->drawTextBox($time, 20, $baseh, 'C', 'M', 0);
            }
            ...
            $pdf->SetXY(10, 22);
            $pdf->drawTextBox('Zeit', 20, 4, 'C', 'M', 0);
            $pdf->SetXY($this->pw - 30, 22);
            $pdf->drawTextBox('Zeit', 20, 4, 'C', 'M', 0);
            
            $pdf->SetXY(10, 26);
            $pdf->drawTextBox($this->data->camp->job_name, 20, 4, 'C', 'M', 0);
            $pdf->SetXY($this->pw - 30, 26);
            $pdf->drawTextBox($this->data->camp->job_name, 20, 4, 'C', 'M', 0);
        }
        
        
        public function page_content($pdf, $subcamp, $start_row, $end_row)
        {
            $date = new c_date();
        ...
        public function time_mm($time)
        {
            ...
            if ($time <= 120) {
                return $time / 60 * $baseh;
            }
            if ($time > 120 && $time < 1080) {
                return ($time / 30 - 2) * $baseh;
            }
            if ($time >= 1080) {
                return ($time / 60 + 16) * $baseh;
            }
        }
