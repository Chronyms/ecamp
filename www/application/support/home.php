 ...
 */
    ...
    if (isset($_REQUEST['search']) && $_REQUEST['search'] == 1) {
        ...
        if (!empty($camp_id)) {
            $search_arg[] = " camp.id 	LIKE '$camp_id%' ";
        }
        if (!empty($camp_desc)) {
            $search_arg[] = " camp.name LIKE '$camp_desc%' ";
        }
        if (!empty($user_id)) {
            $search_arg[] = " user.id 	LIKE '$user_id%' ";
        }
        if (!empty($user_desc)) {
        ...
        if (!empty($camp_id)) {
            $select_arg[] = " (camp.id 		LIKE '$camp_id%')*5 + 	(camp.id 	LIKE '$camp_id')*75 ";
        }
        if (!empty($camp_desc)) {
            $select_arg[] = " (camp.name 	LIKE '$camp_desc%')*2 + (camp.name 	LIKE '$camp_desc')*30 ";
        }
        if (!empty($user_id)) {
            $select_arg[] = " (user.id 		LIKE '$user_id%')*1 + 	(user.id 	LIKE '$user_id')*15 ";
        }
        if (!empty($user_desc)) {
    ...
    while ($camp = mysqli_fetch_assoc($result)) {
        ...
        if (mysqli_num_rows($is_supported)) {
            $camp['supported'] = 1;
        } else {
            $camp['supported'] = 0;
        }
