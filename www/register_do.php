 ...
 */
    ...
    if (!$captcha->isValid($_POST['g-recaptcha-response'])) {
        header('location: register.php?msg=Bitte CAPTCHA richtig abschreiben!');
        die();
    }
    
    if ($_REQUEST[ 'Login' ] == "") {
        header('location: register.php?msg=eMail - Adresse muss angegeben werden!');
        die();
    }
    
    if (!filter_var($_REQUEST[ 'Login' ], FILTER_VALIDATE_EMAIL)) {
        header('location: register.php?msg=Eine gültige eMail - Adresse muss angegeben werden!');
        die();
    }
    
    if ($_REQUEST[ 'Passwort1' ] == "") {
        header('location: register.php?msg=Passwort muss angegeben werden!');
        die();
    }
    
    if ($_REQUEST[ 'Passwort2' ] == "") {
        header('location: register.php?msg=Wiederholung muss angegeben werden!');
        die();
    }
    
    if ($_REQUEST[ 'firstname' ] == "") {
        header('location: register.php?msg=Vorname muss angegeben werden!');
        die();
    }
    
    if ($_REQUEST[ 'surname' ] == "") {
        header('location: register.php?msg=Nachname muss angegeben werden!');
        die();
    }
    ...
    if ($pw1 != $pw2) {
        header('location: register.php?msg=Passwort unstimmig');
        die();
    }
    ...
    if (mysqli_num_rows($result)) {
        header('location: register.php?msg=eMail-Adresse ist bereits registriert');
        die();
    }
